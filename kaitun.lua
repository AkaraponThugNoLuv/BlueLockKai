...
Frame.Size = UDim2.new(1, 0, 1, 50) -- ปรับขนาดให้เต็มหน้าจอ
Frame.Position = UDim2.new(0, 0, -0.05, 0) -- ขยายให้ปิดแทบบนสุด
Frame.BackgroundColor3 = Color3.new(0, 0, 0)
Frame.Parent = ScreenGui

-- โหลดรูปภาพจาก Asset ID
local ImageLabel = Instance.new("ImageLabel")
ImageLabel.Size = UDim2.new(0.3, 0, 0.3, 0)
ImageLabel.Position = UDim2.new(0.35, 0, 0.02, 0)
ImageLabel.BackgroundTransparency = 1
ImageLabel.Image = "rbxassetid://127928059810974"
ImageLabel.Parent = ScreenGui

-- ดึงค่าของผู้เล่น
local player = game.Players.LocalPlayer

-- ฟังก์ชันสร้างป้ายข้อความ
local function createTextLabel(text, position)
    local label = Instance.new("TextLabel")
    label.Size = UDim2.new(0.6, 0, 0.07, 0) -- ขยายขนาดให้ตัวอักษรใหญ่ขึ้น
...
-- สร้างป้ายข้อความสำหรับแต่ละค่า
local playerNameLabel = createTextLabel("Player: " .. player.Name, UDim2.new(0.2, 0, 0.3, 0))
local moneyLabel = createTextLabel("Money: 0", UDim2.new(0.2, 0, 0.38, 0))
local spinsLabel = createTextLabel("Spins: 0", UDim2.new(0.2, 0, 0.46, 0))
local flowSpinsLabel = createTextLabel("Flow Spins: 0", UDim2.new(0.2, 0, 0.54, 0))
local levelLabel = createTextLabel("Level: 0", UDim2.new(0.2, 0, 0.62, 0))

-- อัปเดตค่าทุกๆ 1 วินาที
spawn(function()
    while true do
        moneyLabel.Text = "Money: " .. player.ProfileStats.Money.Value
        spinsLabel.Text = "Spins: " .. player.ProfileStats.Spins.Value
        flowSpinsLabel.Text = "Flow Spins: " .. player.ProfileStats.FlowSpins.Value
        levelLabel.Text = "Level: " .. player.ProfileStats.Level.Value
        wait(1)
    end
end)

-- ฟังก์ชันเปิด/ปิด UI เมื่อกดรูปภาพ
